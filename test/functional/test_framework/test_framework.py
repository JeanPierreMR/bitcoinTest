...
TMPDIR_PREFIX = "cscoin_func_test_"
...
class CScoinTestMetaClass(type):
    """Metaclass for CScoinTestFramework.

    Ensures that any attempt to register a subclass of `CScoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'CScoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("CScoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("CScoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class CScoinTestFramework(metaclass=CScoinTestMetaClass):
    """Base class for a cscoin test script.

    Individual cscoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave cscoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop cscoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use cscoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_cscoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "cscoind" + config["environment"]["EXEEXT"],
        )
        fname_cscoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "cscoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_cscoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "cscoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.cscoind = os.getenv("CSCOIND", default=fname_cscoind)
        self.options.cscoincli = os.getenv("CSCOINCLI", default=fname_cscoincli)
        self.options.cscoinutil = os.getenv("CSCOINUTIL", default=fname_cscoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: cscoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("CScoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'cscoind', self.options.cscoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'cscoin-cli', self.options.cscoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.cscoinconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a cscoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple cscoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as cscoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    cscoind=self.options.cscoind,
                    cscoin_cli=self.options.cscoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in cscoin.conf
    ...
    def skip_if_no_cscoind_tracepoints(self):
        """Skip the running test if cscoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("cscoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_cscoind_zmq(self):
        """Skip the running test if cscoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("cscoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if cscoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("cscoin-wallet has not been compiled")

    def skip_if_no_cscoin_util(self):
        """Skip the running test if cscoin-util has not been compiled."""
        if not self.is_cscoin_util_compiled():
            raise SkipTest("cscoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if cscoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("cscoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether cscoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether cscoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_cscoin_util_compiled(self):
        """Checks whether cscoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_CSCOIN_UTIL")
