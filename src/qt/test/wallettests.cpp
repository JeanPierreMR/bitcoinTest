...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<CScoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = CScoinUnits::formatWithUnit(unit, expected_balance, false, CScoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_cscoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_cscoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_cscoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    // Amount input
    CScoinAmountField* amountInput = receiveCoinsDialog.findChild<CScoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_cscoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
