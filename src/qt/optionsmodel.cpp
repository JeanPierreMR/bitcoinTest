...
/** Call node.updateRwSetting() with CScoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because CScoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/cscoin/cscoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because cscoin 22.x will not try to read these.
...
//! Convert enabled/size values to cscoin -prune setting.
...
//! Get pruning enabled value to show in GUI from cscoin -prune setting.
...
//! Get pruning size value to show in GUI from cscoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCScoinUnit")) {
        settings.setValue("DisplayCScoinUnit", QVariant::fromValue(CScoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayCScoinUnit");
    if (unit.canConvert<CScoinUnit>()) {
        m_display_cscoin_unit = unit.value<CScoinUnit>();
    } else {
        m_display_cscoin_unit = CScoinUnit::BTC;
        settings.setValue("DisplayCScoinUnit", QVariant::fromValue(m_display_cscoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if cscoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_cscoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<CScoinUnit>() == m_display_cscoin_unit) return;
    m_display_cscoin_unit = new_unit.value<CScoinUnit>();
    QSettings settings;
    settings.setValue("DisplayCScoinUnit", QVariant::fromValue(m_display_cscoin_unit));
    Q_EMIT displayUnitChanged(m_display_cscoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/cscoin/cscoin/pull/8273
    ...
    // (https://github.com/cscoin-core/gui/issues/567).
