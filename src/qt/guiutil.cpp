...
#include <qt/cscoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CScoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CScoinAddressEntryValidator(parent));
    widget->setCheckValidator(new CScoinAddressCheckValidator(parent));
}
...
bool parseCScoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no cscoin: URI
    if(!uri.isValid() || uri.scheme() != QString("cscoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!CScoinUnits::parse(CScoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseCScoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCScoinURI(uriInstance, out);
}

QString formatCScoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("cscoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CScoinUnits::format(CScoinUnit::BTC, info.amount, false, CScoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCScoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", CSCOIN_CONF_FILENAME));
    ...
    /* Open cscoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CScoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("CScoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for CScoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("cscoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a cscoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CScoin\n";
        else
            optionFile << strprintf("Name=CScoin (%s)\n", chain);
