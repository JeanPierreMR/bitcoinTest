...
const char * const CSCOIN_CONF_FILENAME = "cscoin.conf";
const char * const CSCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cscoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", CSCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\CScoin
    // macOS: ~/Library/Application Support/CScoin
    // Unix-like: ~/.cscoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/CScoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", CSCOIN_CONF_FILENAME);
