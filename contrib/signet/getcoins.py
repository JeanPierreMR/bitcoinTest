...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing cscoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='cscoin-cli', help='cscoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='CScoin address to which the faucet should send')
...
parser.add_argument('cscoin_cli_args', nargs='*', help='Arguments to pass on to cscoin-cli (default: -signet)')

args = parser.parse_args()

if args.cscoin_cli_args == []:
    args.cscoin_cli_args = ['-signet']


def cscoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.cscoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = cscoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = cscoin_cli(['getnewaddress', 'faucet', 'bech32'])
