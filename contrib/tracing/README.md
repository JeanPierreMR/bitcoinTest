...
Tracing (USDT) support for CScoin Core on Linux using. For more information on
USDT support in CScoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `cscoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `cscoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[CScoin Core USDT documentation] on how to list available tracepoints in your
`cscoind` binary.

[CScoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/cscoind
```
...
CScoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/cscoind
```
...
In a different terminal, starting CScoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/cscoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/cscoind
```
