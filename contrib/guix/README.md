# Bootstrappable CScoin Core Builds

This directory contains the files necessary to perform bootstrappable CScoin
Core builds.
...
To build CScoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      CScoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/cscoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/cscoin-core/guix.sigs).
