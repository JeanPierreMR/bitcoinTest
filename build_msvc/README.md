Building CScoin Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build CScoin Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build CScoin Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build CScoin Core with the GUI, a static build of Qt is required.
...
To build CScoin Core without Qt, unload or disable the `cscoin-qt`, `libcscoin_qt` and `test_cscoin-qt` projects.
...
```cmd
msbuild build_msvc\cscoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/cscoin.sln` file in Visual Studio.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make CScoin Core more secure. When building CScoin using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `cscoind` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/cscoind.exe
```
