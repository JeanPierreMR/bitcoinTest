# User-space, Statically Defined Tracing (USDT) for CScoin Core

CScoin Core includes statically defined tracepoints to allow for more
...
**Note**: CScoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to CScoin Core
...
Multiple tools can list the available tracepoints in a `cscoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in CScoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/cscoind
stap net        outbound_message 0x0000000000107c05 /src/cscoind
stap validation block_connected  0x00000000002fb10c /src/cscoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in CScoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/cscoind | grep NT_STAPSDT -A 4 -B 2
