CScoin Core version 0.19.0.1 is now available from:

  <https://cscoincore.org/bin/cscoin-core-0.19.0.1/>
...
CScoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use CScoin Core on unsupported systems.
...
to open the CScoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the CScoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/cscoin/cscoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running CScoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of CScoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `cscoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile CScoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in CScoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of CScoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in CScoin Core,
  ...
  This prevents CScoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of CScoin Core and nodes that have manually enabled BIP37 support,
...
- The `testnet` field in `cscoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how CScoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  CScoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, CScoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 CScoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in CScoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@CScoin AEAD (jonasschnelli)
...
- #15639 cscoin-wallet tool: Drop libcscoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in cscoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "cscoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of cscoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from cscoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_cscoin to auto generated project files (sipsorcery)
...
- #15773 Add CScoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_cscoin output (practicalswift)
...
- #15896 feature_filelock, interface_cscoin_cli: Use PACKAGE_NAME in messages rather than hardcoding CScoin Core (luke-jr)
...
- #16277 Suppress output in test_cscoin for expected errors (gertjaap)
...
- #16489 log: harmonize cscoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (cscoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example cscoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBCSCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update cscoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in cscoin.conf (fanquake)
...
- #16621 add default cscoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the cscoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, cscoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/cscoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/cscoin/cscoin/).
