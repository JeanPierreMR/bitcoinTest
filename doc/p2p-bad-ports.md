When CScoin Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no CScoin node is listening,
or one where a service is listening that is not related to the CScoin network.
As a result, this service may occasionally get connection attempts from CScoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by CScoin Core,
trying to connect because it thinks there is a CScoin node on that
...
Below is a list of "bad" ports which CScoin Core avoids when choosing a peer to
...
[pull/23306](https://github.com/cscoin/cscoin/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/cscoin/cscoin/pull/23542)
