# PSBT Howto for CScoin Core

Since CScoin Core 0.17, an RPC interface exists for Partially Signed CScoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/cscoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for CScoin transactions that are not fully signed
...
#### Multisig with multiple CScoin Core instances
...
CScoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `cscoin-cli` is used.
