# CJDNS support in CScoin Core

It is possible to run CScoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of CScoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the CScoin network and individual nodes.
...
before it will work with your CScoin Core node. This is described in steps
...
## Run CScoin Core with CJDNS

Once you are connected to the CJDNS network, the following CScoin Core
...
When enabled, this option tells CScoin Core that it is running in an
...
IPv6 local network. This helps CScoin Core perform better address management:
...
CJDNS support was added to CScoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `cscoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/cscoin/cscoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in CScoin Core

There are several ways to see your CJDNS address in CScoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `cscoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `cscoin-cli getpeerinfo`).
